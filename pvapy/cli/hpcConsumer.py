...
def main():
    ...
    parser.add_argument('-im', '--input-mode', dest='input_mode', default='pva', help='Input mode. This parameter determines how the input data will be received. Valid options are "pva" (default, indicates that a channel monitor will be receiving updates from the remote input PVA channel), "pvas" (indicates that remote clients will be updating the input channel hosted on the local PVA server), and "rpc" (indicates that remote clients will be updating the input channel hosted on the local RPC server). The default "pva" option is appropriate when receiving input from external PVA servers, or from consumers producing output in a default "pvas" mode. The "pvas" option should be used when receiving data produced by a consumer using the "pva" output mode, while the "rpc" option should be used when receiving data produced by a consumer using the "rpc" output mode.')
