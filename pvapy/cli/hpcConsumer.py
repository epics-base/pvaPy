...
def main():
    ...
    parser.add_argument('-sqs', '--server-queue-size', type=int, dest='server_queue_size', default=0, help='Server queue size (default: 0). This setting is only relevant for the default "pva" input mode, and it will increase memory usage on the server side, but may help prevent missed PV updates.')
    parser.add_argument('-rqs', '--receiver-queue-size', type=int, dest='receiver_queue_size', default=-1, help='Data receiver queue size (default: -1); if < 0, PV updates will be processed immediately without copying them into PvObjectQueue; if >= 0, PvObjectQueue will be used for receving PV updates (value of zero indicates infinite queue size).')
    parser.add_argument('-ao', '--accumulate-objects', type=int, dest='accumulate_objects', default=-1, help='Number of objects to accumulate in the data receiver queue before they can be processed (default: -1); if <= 0 the processing happens regarding of the current receiver queue length. This option is ignored unless receiver queue size is set (i.e., >= 0). Note that after accumulation timeout, all objects in the queue will be processed.')
    parser.add_argument('-at', '--accumulation-timeout', type=float, dest='accumulation_timeout', default=1, help='Time period since last received item after which objects in the data receiver queue will be processed regardless of the current queue length (default: 1 second). This option is ignored unless receiver queue size is set (i.e, >= 0) and if number of accumulated objects is not set (i.e., <= 0).')
    ...
    parser.add_argument('-rs', '--report-stats', dest='report_stats', default='all', help='Comma-separated list of statistics subsets that should be reported (default: all); possible values: receiver, publisher, queue, processor, user, all.')
    ...
    controller = ControllerClass(
        ...
        inputArgs=args.input_args,
        outputChannel=args.output_channel,
        outputMode=args.output_mode,
        outputArgs=args.output_args,
        ...
        receiverQueueSize=args.receiver_queue_size,
