...
class UserDataProcessor:
    ...
    **UserDataProcessor(configDict={})**
    ...
    def getOutputPvObjectType(self, pvObject):
        '''
        Method invoked at processing startup that defines PVA structure for
        the output (processed) PvObject when the "pvas" (PVA server) output
        mode is used. This method is called immediately after receiving the
        first input channel update.
        
        There is no need to override this method if the structure of input and
        output objects are the same in the "pvas" (PVA server) output mode, if
        either "pva" (PVA client), "rpc" (RPC client), or "ejfat" (EJFAT
        sender) output modes are used, or if the application will not publish
        processing output at all.
    ...
    def updateOutputChannel(self, pvObject):
        ...
        '''
        if not self.outputChannel or not self.dataPublisher:
            return
        try:
            self.dataPublisher.updateOutputChannel(pvObject)
        except Exception as ex:
            self.logger.error(f'Cannot update output channel {self.outputChannel}: {ex}')
