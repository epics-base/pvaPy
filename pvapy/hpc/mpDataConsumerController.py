...
class MpDataConsumerController(SystemController):
    ...
    **MpDataConsumerController(inputChannel, inputMode='pva', inputArgs=None, outputChannel=None, outputMode='pvas', outputArgs=None, statusChannel=None, controlChannel=None, idFormatSpec=None, processorFile=None, processorClass=None, processorArgs=None, objectIdField='uniqueId', objectIdOffset=0, fieldRequest='', skipInitialUpdates=1, reportStatsList='all', logLevel=None, logFile=None, disableCurses=False, consumerId=1, nConsumers=1, serverQueueSize=0, receiverQueueSize=-1, accumulateObjects=-1, accumulationTimeout=1, distributorPluginName='pydistributor', distributorGroup=None, distributorSet=None, distributorTrigger=None, distributorUpdates=None, nDistributorSets=1, metadataChannels=None)**
    ...
    :Parameter: *inputMode* (str) - Input mode. This parameter determines how the input data will be received. Valid options are "pva" (default, indicates that a channel monitor will be receiving updates from the remote input PVA channel), "pvas" (indicates that remote clients will be updating the input channel hosted on the local PVA server), "rpc" (indicates that remote clients will be updating the input channel hosted on the local RPC server), and "ejfat" (indicates that input data will be received via EJFAT). The default "pva" option is appropriate when receiving input from external PVA servers, or from consumers producing output in a default "pvas" mode. The "pvas" option should be used when receiving data produced by a consumer using the "pva" output mode, while the "rpc" or "ejfat" options should be used when receiving data produced by a consumer using the "rpc" or "ejfat" output mode, respectively.
    :Parameter: *inputArgs* (str) - JSON-formatted string that can be converted into dictionary and used for initializing input data receiver.
    ...
    :Parameter: *outputMode* (str) - Output mode. This parameter determines how the output data will be published. Valid options are "pvas" (default, indicates that the output channel will be hosted on the local PVA server), "pva" (indicates that a local client will be updating an output channel hosted on a remote PVA server), "rpc" (indicates that a local client will be updating an output channel hosted on a remote RPC server), and "ejfat" (indicates that output data will be published via EJFAT). The default "pvas" option is appropriate when the remote clients or consumers are expected to monitor the output channel hosted by the local PVA server. The "pva" option should be used when publishing data to an output channel hosted on a remote external PVA server, or on another consumer expecting input in the "pvas" mode. The "rpc" option should be used when publishing data to an output channel hosted on a remote RPC server, or on another consumer expecting input in the "rpc" mode. The "ejfat" options should be used for publishing data via EJFAT. All options other than "pvas" require remote servers to be ready to receive data.
    :Parameter: *outputArgs* (str) - JSON-formatted string that can be converted into dictionary and used for initializing output data publisher.
    ...
    def start(self):
        ...
        for consumerId in self.consumerIdList:
            ...
            mpProcess = mp.Process(target=mpdcController,
            args=(requestQueue, responseQueue, self.inputChannel, self.inputMode, self.inputArgs, self.outputChannel, self.outputMode, self.outputArgs, self.statusChannel, self.controlChannel, self.idFormatSpec, self.processorFile, self.processorClass, self.processorArgs, self.objectIdField, self.objectIdOffset, self.fieldRequest, self.skipInitialUpdates, self.reportStatsList, self.logLevel, self.logFile, self.disableCurses, consumerId, self.nConsumers, self.serverQueueSize, self.receiverQueueSize, self.accumulateObjects, self.accumulationTimeout, self.distributorPluginName, self.distributorGroup, self.distributorSet, self.distributorTrigger, self.distributorUpdates, self.nDistributorSets, self.metadataChannels,))
    ...
    def getCombinedSystemStats(self, statsDict):
        combinedQueueStats = {}
        combinedReceiverStats = {}
        combinedPublisherStats = {}
        combinedProcessorStats = {}
        for consumerId in self.consumerIdList:
            consumerStats = statsDict[consumerId]
            queueStats = consumerStats.get('queueStats', {})
            combinedQueueStats.update(StatsUtility.addKeyValues(queueStats, combinedQueueStats))
            receiverStats = consumerStats.get('receiverStats', {})
            combinedReceiverStats.update(StatsUtility.addKeyValues(receiverStats, combinedReceiverStats))
            processorStats = consumerStats.get('processorStats', {})
            combinedProcessorStats.update(StatsUtility.addKeyValues(processorStats, combinedProcessorStats, keys=['errorRate', 'missedRate', 'processedRate', 'nErrors', 'nMissed', 'nProcessed']))
            publisherStats = consumerStats.get('publisherStats', {})
            combinedPublisherStats.update(StatsUtility.addKeyValues(publisherStats, combinedPublisherStats))
        ...
        return {'receiverStats' : combinedReceiverStats, 'publisherStats' : combinedPublisherStats, 'processorStats' : combinedProcessorStats, 'queueStats' : combinedQueueStats}
