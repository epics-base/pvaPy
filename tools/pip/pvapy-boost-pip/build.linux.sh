...
BOOST_VERSION_NUMBER=`cat boost/version.hpp  | grep BOOST_VERSION | grep
\#define | grep -v HPP | awk '{print $NF}'`
if [ $BOOST_VERSION_NUMBER -gt 108100 -o ! -z "$PVAPY_USE_CPP11" ] ; then
    echo "Building boost python, using C++11"
    #./b2 -std=c++11 --with-python python-debugging=off threading=multi variant=release address-model=64 || exit 1
